...
class MatrixApp {
    ...
    private async refreshTable() {
        ...
        try {
            ...
            this.currentUUID = result.uuid; //session uuid 

            console.log("UUid", this.currentUUID);
            //Создаём пустую таблицу размера списка
            ...
            try {
                ...
                if (savedData.uuid === this.currentUUID &&
                    ...
                    this.matrix = savedData.data.map(row =>
                        row.map(n => OPTIONS_MAP[n as 0 | 1 | 2])
                    );
    ...
    private renderTable() {
        ...
        table.innerHTML = ""; // очищаем полностью
...
export function init() {
    console.log("Matrix page initializing...");
    new MatrixApp();
}